...
class Convolution(val KW: Int, val IH: Int, val IW: Int, val IC: Int, val OC: Int, val eps: Double = 0.001, val rho1: Double = 0.9, val rho2: Double = 0.999) extends Layer {
  val OH = IH - KW + 1
  val OW = IW - KW + 1
  var Vs = List[Array[T]]()
...
class Convolution2(val kw: Int, //カーネルの幅
                   val H: Int, //入力の高さ
                   val W: Int, //入力の幅
                   val I: Int, //入力チャネル数
                   val O: Int, //出力チャネル数
  ...
  def backward(G: Array[Double]) = {
    ...
    for (i <- 0 until I; j <- 0 until H; k <- 0 until W) {
      var s_v = 0d
      for (l <- 0 until h_d; m <- 0 until kw; n <- 0 until w_d; p <- 0 until kw; q <- 0 until O) {
        if (l + m == j && n + p == k) {
...
class lstm(
  ...
  var iList = new Stack[Array[Double]]()
  ...
  val t = new Tanh()
  hList.push(new Array[Double](Out))
  cList.push(new Array[Double](Out))
  ...
  rList.push(new Array[Double](Out))
